...
export function restoreLogicalBackup(payload, userId) {
  const isNexus = payload && (payload.app === 'nexus-crm' || payload.app === 'campus-crm'); // campus — совместимость со старыми бэкапами
  if (!payload || !isNexus || !payload.tables) {
    throw new Error('Это не файл бэкапа NexusCRM');
  }
