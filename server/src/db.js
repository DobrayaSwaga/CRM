...
export function refreshSearchText(contactId) {
  const c = db.prepare('SELECT last_name, first_name, middle_name, phone, phone_normalized, email, city, school FROM contacts WHERE id = ?').get(contactId);
  if (!c) return;
  const text = [c.last_name, c.first_name, c.middle_name, c.phone, c.phone_normalized, c.email, c.city, c.school]
