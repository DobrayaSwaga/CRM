    ...
    <div className="space-y-4">
        ...
        <div className="space-y-1.5">
          {rules.map(r => (
            <div key={r.id} className="flex items-center gap-3 glass rounded-xl px-3.5 py-2.5">
              ...
              <Badge color={r.points >= 50 ? '#f87171' : r.points >= 30 ? '#fbbf24' : '#7dd3fc'}>+{r.points}</Badge>
      ...
      <Modal open={showRule} onClose={() => setShowRule(false)} title="Новое правило скоринга">
        <div className="space-y-3">
          ...
          <Field label="Баллы"><Input type="number" value={form.points} onChange={e => setForm((f: any) => ({ ...f, points: e.target.value }))} /></Field>
          <div className="flex justify-end gap-2">
