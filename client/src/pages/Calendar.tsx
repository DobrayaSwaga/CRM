              ...
              {cells.map((day, i) => {
                ...
                return (
                  <div key={i} className={`border-b border-r border-white/4 p-1.5 min-h-[90px] ${isToday(day) ? 'bg-indigo-500/8' : ''}`}>
                    <div className={`text-[11px] font-semibold mb-1 inline-flex w-6 h-6 items-center justify-center rounded-lg ${isToday(day) ? 'bg-indigo-500 text-white shadow-lg shadow-indigo-500/40' : 'text-slate-500'}`}>
                      {day}
                    </div>
